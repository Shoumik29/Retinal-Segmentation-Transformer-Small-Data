'''
Responsibility
	- Build and return Swin Transformer Encoder

Took help from https://github.com/sayakpaul/swin-transformers-tf/blob/main/swins
'''
...
class PatchMerging(layers.Layer):
	...
	def call(self, x):
		...
		x = tf.reshape(x, shape=[-1, (H // 2) * (W // 2), 4 * C])  # [B, H/2*W/2, 4*C]
...
def SwinTransformer(x, img_size=224,
		patch_size=4,
		embed_dim=96,			#96,
		depths=(2, 2, 6, 2),
	...
	if ape:
		absolute_pos_embed = tf.Variable(
			tf.zeros((1, num_patches, embed_dim)),
			trainable=True,
			name="absolute_pos_embed",
		)
	else:
		absolute_pos_embed = None
	pos_drop = layers.Dropout(drop_rate)

	# build layers
	if not isinstance(embed_dim, (tuple, list)):
		embed_dim = [
			int(embed_dim * 2 ** i) for i in range(num_layers)
		]
	embed_out_dim = embed_dim[1:] + [None]
	head_dim = to_ntuple(num_layers)(head_dim)
	window_size = to_ntuple(num_layers)(window_size)
	...
	# layers	
	x = projection(x)
	'''x = layers.Conv2D(filters=96, kernel_size=(2, 2), strides=(2, 2), padding="VALID", kernel_initializer="lecun_normal")(x)
	x = layers.Reshape(((int(x.shape[1]) * int(x.shape[1])), x.shape[-1]))(x)'''
	...
	x = layers.LayerNormalization(epsilon=1e-5, name='swin_last_layer_norm')(x)  # [B, L, C]
...
Currently we do not need the attention score 
'''
